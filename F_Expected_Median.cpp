...
// int findAns(vll& tan,ll idx,ll total,ll count,ll n,ll k){
//     if(idx>=n || total==k){
//         if(count>=(k+1)/2 && (total==k)) return 1;
        
//         return 0;
//     }
//     if((n-idx)<(k-total)) return 0;

//     ll not_take=findAns(tan,idx+1,total,count,n,k);
//     if(tan[idx]==1) count++;
//     ll take=findAns(tan,idx+1,total+1,count,n,k);
...
ll nCr(ll n,ll r){
    ...
    return ((fact[n]*RevFac[r])%mod)*RevFac[n-r]%mod;
}
void solve() {
    ll n,k;
    cin>>n>>k;
...
int main() {
   ...
   fact[0] = RevFac[0] = 1;
        for (int i=1 ; i<N ; i++) {
            ...
            RevFac[i] = fast_pow(fact[i], mod-2);
        }
    while (T--) {
        solve();
    }
