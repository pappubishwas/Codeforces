...
void solve() {
    ll n, m;
    cin >> n ;
    vector<int> arr(2*n),prefix(2*n+1,0);
    for (ll i=1;i<=2*n;i++) {
        cin >> arr[i-1];
        prefix[i]=prefix[i-1]+ ((i%2==1 ) ?   -arr[i-1]: arr[i-1]);
    }
    vector<ll> ans;
    ans.push_back(prefix[2*n]);
    ll extra=0;
    for(int i=1;i<n;i++){
        ll t=abs(prefix[2*n-i]-prefix[i]);
        extra+= arr[2*n-i]- arr[i-1];
        ans.push_back(t+extra);
    }
    for(auto x:ans){
        cout<<x<<" ";
    }
