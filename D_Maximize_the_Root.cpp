...
int dfs(int node, int parent, vector<vector<int>> &adj, vector<int> &pap)
{
    int curr = INF;
    for (int &child : adj[node])
    {
        if (child != parent)
            curr = min(curr, dfs(child, node, adj, pap));
    }
    if (curr == INF)
        return pap[node];
    if(pap[node] > curr) return curr;    
    return (curr + pap[node]) / 2;
}
...
void solve()
{
    ...
    for (int &child : adj[1])
    {
        ans = min(ans, dfs(child, 1, adj, pap));
    }
    cout << ans + pap[1] << endl;
}
