...
void dfs(int parant,int child,ll parantval,vector<bool> &visited,vector<vector<ll>> &adj,vector<ll> &ans,vector<ll> &v){
    if(parantval>v[parant]){
        ans[child]=parantval-v[parant]+v[child];
    }else{
        ans[child]=v[child];
    }
    visited[child]=true;
    parantval=ans[parant];
    for(auto &nextChild: adj[child]){
        if(!visited[nextChild]){
            dfs(child,nextChild,parantval,visited,adj,ans,v);
        }
    }
}
void solve(){
