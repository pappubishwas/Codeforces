...
void solve() {
    ll n,k,m;
    cin >> n>>k;
    vector<ll> pap(n),tan(n);
    ll mx=LLONG_MIN,mn=LLONG_MAX;
    ll sum=-1,cnt=0;
    for(auto &x:pap){
        cin>>x;
    }
    vector<pair<ll,ll>> vct,ng;
    for(auto &x:tan){
        cin>>x;
    ...
    for(ll i=0;i<n;i++){
        if(tan[i]!=-1)
        vct.push_back({pap[i],tan[i]});
        else{
            ng.push_back({pap[i],tan[i]});
        }
    }
    sort(vct.begin(), vct.end(), [](const pair<ll, ll> &a, const pair<ll, ll> &b) {
        if (a.first == b.first) return a.second < b.second;
        return a.first < b.first;
    });
    sort(ng.begin(), ng.end(), [](const pair<ll, ll> &a, const pair<ll, ll> &b) {
        if (a.first == b.first) return a.second < b.second;
        return a.first < b.first;
    });
    ...
    if(n==cnt){
        mx=ng[0].first+k;
        ll last=mx-ng[n-1].first;
        if(last<0) cout<<0<<endl;
        else cout<<last+1<<endl;
    }else{
        ll sm=-1;
        for(auto &p:vct){
            if(sm!=-1 && sm!=(p.first+p.second)){
                cout<<0<<endl;
                return;
            }
            sm=p.first+p.second;
        ...
        ll last=vct[0].first+vct[0].second;
        ll j=ng.size();
        ll ft=last-ng[0].first;
        ll lt=last-ng[j-1].first;
        if(ft>k || lt<0) cout<<0<<endl;
        else cout<<1<<endl;
    }
}
