...
{  
   ...
   while(T--)
   {
    ...
    f(i,0,n) cin>>a[i];
    f(i,0,n) cin>>b[i];
    mna=*min_element(begin(a),end(a));
    mnb=*min_element(begin(b),end(b));
    ll sr=0,sc=0;
    sr=accumulate(begin(a),end(a),0ll);
    sc=accumulate(begin(b),end(b),0ll);

    cout<<min((sr+mnb*n),(sc+mna*n))<<endl;
   }
    return 0;
}
