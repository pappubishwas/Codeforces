...
void solve(){
    ...
    f(i,2,n){
        ll j=i-1;
        if(abs(tan[j]-tan[j+1])<abs(tan[j]-tan[j-1])) prefix[i]=prefix[j]+1;
        else prefix[i]=prefix[j]+abs(tan[j]-tan[j+1]);
    }
    rf(i,n-3,0){
        ll j=i+1;
        if(abs(tan[j]-tan[j+1])>abs(tan[j]-tan[j-1])) suffix[i]=suffix[i+1]+1;
        else suffix[i]=suffix[i+1]+abs(tan[j]-tan[j-1]);
    }
    // f(i,0,n)cout<<prefix[i]<<" ";
    // f(i,0,n)cout<<suffix[i]<<" ";
    cin>>q;
    f(i,0,q){
        ll x,y;
        cin>>x>>y;
