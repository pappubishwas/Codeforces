...
void solve() {
    ll n;
    cin >> n;
    vector<ll> pap(n + 1), tan(n + 1);

    for (ll i = 1; i <= n; i++) cin >> pap[i];
    for (ll i = 1; i <= n; i++) cin >> tan[i];
    ...
    for (ll i = 2; i <= n; i++) {
        if (pap[i] == tan[i]) {
            ...
            if (mp.find(pap[i]) != mp.end()) {
                for (auto &p : mp[pap[i]]) {
                    ll sm = p.first + p.second;
                    if (sm % 2 != (i + 1) % 2 || (i - p.first) > 1) {
                        res = max(p.first, res);
                    }
                }
            }
            if (mp.find(tan[i]) != mp.end()) {
                for (auto &p : mp[tan[i]]) {
                    ll sm = p.first + p.second;
                    if (sm % 2 != (i + 2) % 2 || (i - p.first) > 1) {
                        res = max(p.first, res);
                    }
                }
            }
        ...
        while (mp[pap[i]].size() > 2) mp[pap[i]].erase(mp[pap[i]].begin());
        while (mp[tan[i]].size() > 2) mp[tan[i]].erase(mp[tan[i]].begin());
    }
