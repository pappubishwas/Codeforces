...
{  
   ...
   while(T--)
   {
        ...
        vll a(n),suffixsum(n);
        map<ll, ll> mp;
        ll mx1 = LONG_MIN, mn2 = LONG_MAX, res = LONG_MIN,prefixsum=0;
        f(i,0,n) cin>>a[i];
        sort(begin(a),end(a));
        ...
        while (i<=n && i<=k)
        {
            ...
            if(i+x<n){
                currsum=suffixsum[i+x-1]-prefixsum;
                lastsum=suffixsum[n-1]-suffixsum[i+x-1];
            }else{
                currsum=suffixsum[n-1]-prefixsum;
            }
            res=max(res,(-currsum+lastsum));
            prefixsum+=a[i];
            i++;
        }
