...
void solve() {
    ll n, k,l,r;
    cin>>n;
    vll tan(n+10),prefix(n+10),b(n+10),finalans(n+10,n);
    f(i,1,n+1){
        cin>>tan[i];
        prefix[i]=prefix[i-1]+tan[i];
    }
    ...
    f(i,1,n+1){
        ...
        {
            ll mid=(left+right)/2;
            ll rangesum=prefix[mid]-prefix[i];
        ...
        if (n >= left) {
            finalans[i] = min(finalans[i], (left - i));
        }
    ...
    f(i,1,n+1){
        ...
        while (right>left)
        {
            ll mid=((left+right+1)/2) -1;
            ll rangesum=prefix[i-1]-prefix[mid];
        ...
        if (left>0) {
            finalans[i] = min(finalans[i],(i-left));
        }
