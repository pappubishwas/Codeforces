...
void solve(){
    ...
    for(auto c:mp){
        ll u=c.second;
        ans+=u*(u-1)*(u-2)/6;  // u Combination 3 //// u!/(3!*(u-3)!)=u*(u-1)*(u-2)/6
        ans+=s*u*(u-1)/2;    // u Combination 2 //// u!/(2!*(u-2)!)=u*(u-1)/2 ,,,, s is the count of smallest element of courrent element
        s+=u;
    }
    cout<<ans<<endl;
}
