...
//O(sqrt(n))  - for finding
//O(sqrt(n)*Log(sqrt(n))) - for sorting
vll factors;
void factors_of_n(long long n)
{
...
{  
   ...
   { 
    ...
    if(val%2==0)
    {
      ...
      val/=2;
      factors_of_n(val);
      ll id=lower_bound(all(factors),x-1)-factors.begin();
      debug(factors)
      ans+=sz(factors)-id;
    }
    debug(ans)
    ...
    if(val%2==0 and x!=1)
    {
      ...
      val/=2;
      factors_of_n(val);
      ll id=lower_bound(all(factors),x)-factors.begin();
      debug(val)
      debug(factors)
    //   ll id2=lower_bound(all(factors),n)-factors.begin();
      ans+=sz(factors)-id;
