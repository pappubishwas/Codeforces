...
string t = "narek";
int findAns(int i, int j, int k, vector<string> &g, vector<vector<vector<int>>> &dp)
{
    if (i >= g.size())
        return -2*k;
    if (dp[i][j][k] != -1)
        return dp[i][j][k];
    int ans = -1e14;
    if (j < g[0].size() && g[i][j] == t[k])
        ans = 1 + findAns(i, j + 1, (k + 1) % 5, g, dp);
    if (j < g[0].size())
    {
        int val = 0;
        if (t.find(g[i][j]) != string::npos)
            val = -1;
        ans = max(ans, val + findAns(i, j + 1, k, g, dp));
    }
    if (j == 0 || j == g[0].size())
    {
        ans = max(ans, findAns(i + 1, 0, k, g, dp));
    }
