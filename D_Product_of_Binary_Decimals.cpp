...
bool is_binary_decimal(int n) {
    if (n == 1) return true;
    if (find(binarydecimal.begin(), binarydecimal.end(), n) != binarydecimal.end()) return true;

    for (int i = 2; i < binarydecimal.size(); i++) {
        if (n % binarydecimal[i] == 0) {
            if (is_binary_decimal(n / binarydecimal[i])) return true;
        }
    }
    return false;
}
...
int main() {
    ...
    cin >> T;
    for (int i = 1; i < 64; ++i) {
        string binary_str = bitset<6>(i).to_string();
        binarydecimal.push_back(stoi(binary_str));
    }
    // for (int i = 0; i < 63; ++i) {
    //     cout<<binarydecimal[i]<<" ";
    // }
