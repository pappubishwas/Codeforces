...
void solve()
{
    ...
    for(auto &it: mp){
        ...
        points.push_back({it.second[0],it.second.back()});
    }
    int sz=points.size();
    vector<int> dp1(sz,0),dp2(sz,0);
    for(int i=1;i<sz;i++){
        dp1[i]=min(dp1[i-1]+abs(points[i][1]-points[i-1][0])+(points[i][1]-points[i][0]),dp2[i-1]+abs(points[i][1]-points[i-1][1])+(points[i][1]-points[i][0]));
        dp2[i]=min(dp2[i-1]+abs(points[i][0]-points[i-1][1])+(points[i][1]-points[i][0]),dp1[i-1]+abs(points[i][0]-points[i-1][0])+(points[i][1]-points[i][0]));
    }
    ans+=min(dp1[sz-1],dp2[sz-1]);
    cout<<ans<<endl;
}
