...
void solve()
{
    ...
    auto get_count = [&](int L, int R) {
        if (L <= R) {
            ...
            return (prefix[2 * k - 1] - (L == 0 ? 0 : prefix[L - 1])) + prefix[R];
        }
    };
    
    int ans = -1;
    
    for(int t = mx; t < mx + k; t++){
        
        int curr = t % (2 * k);
        int start_window = (curr - k + 1 + 2 * k) % (2 * k);
        if (get_count(start_window, curr) == n) {
            ans = t;
            break; 
        }
    }
