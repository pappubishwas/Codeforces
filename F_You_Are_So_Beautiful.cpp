...
void solve(){
    ll n,k,q,ans=0,pos=-1;
    cin>>n;
    map<ll,ll> lastOccurrence,firtOccurrence;
    vll a(n);
    f(i,0,n){
        cin>>a[i];
    ...
    rf(i,n-1,0){
        firtOccurrence[a[i]]=i;
    }
    ll unique_element=0;
    f(i,0,n){
        if(firtOccurrence[a[i]]==i) unique_element++;
        if(lastOccurrence[a[i]]==i) ans+=unique_element;
    }
    cout<<ans<<endl;
}
int main()
